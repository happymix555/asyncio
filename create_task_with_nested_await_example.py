...
async def myPrintAndAddTask( numToPrintInt: int, timeToSleepInt: int = None ):
    ...
    myPrintResult = asyncio.create_task( myPrint( numToPrintInt, timeToSleepInt ), name='myPrint( {} )'.format( numToPrintInt ) )
    
    # await to wait for the result of myPrintResult
    await myPrintResult
    
    # print to inform that we have finished adding task to event loop
    print( 'Task with number: {} added to event loop via myPrintAndAddTask function'.format( numToPrintInt ) + '\n')
...
async def main():
    ...
    firstMyPrintAndAddTask = asyncio.create_task( myPrintAndAddTask( 1, 5 ), name='myPrintAndAddTask( 1 )' )
    ...
    secondMyPrintAndAddTask = asyncio.create_task( myPrintAndAddTask( 2, 1 ), name='myPrintAndAddTask( 2 )' )
